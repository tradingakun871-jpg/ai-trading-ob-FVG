...
#property description "AI Trading OB+FVG Hybrid V3 - local DXY hard gate + Railway AI supervisor, fixed RR 1:2"
...
input double MinInverseCorrelation=-0.15;
input int DxyMaxAgeSeconds=180;
...
double PearsonInverseCorrelation(int bars)
{
   if(g_dxySymbol==""||bars<10)return 2.0;
   MqlRates x[],d[]; ArraySetAsSeries(x,true); ArraySetAsSeries(d,true); int need=bars+1;
   if(CopyRates(g_symbol,PERIOD_M1,1,need,x)<need||CopyRates(g_dxySymbol,PERIOD_M1,1,need,d)<need)return 2.0;
   double sx=0,sd=0,sxx=0,sdd=0,sxd=0; int n=0;
   for(int i=0;i<bars;i++){if(x[i+1].close<=0||d[i+1].close<=0)continue;double rx=(x[i].close-x[i+1].close)/x[i+1].close,rd=(d[i].close-d[i+1].close)/d[i+1].close;sx+=rx;sd+=rd;sxx+=rx*rx;sdd+=rd*rd;sxd+=rx*rd;n++;}
   if(n<10)return 2.0; double num=n*sxd-sx*sd,den=MathSqrt((n*sxx-sx*sx)*(n*sdd-sd*sd)); if(den<=0)return 2.0; return num/den;
}
bool LocalDxyGate(string side,string &reason,double &corr)
{
   corr=2.0;if(!EnableLocalDxyHardGate){reason="DXY_GATE_DISABLED";return true;}if(!EnableDxyFeed||g_dxySymbol==""){reason="WAIT_DXY_SYMBOL";return false;}
   MqlTick dt;if(!SymbolInfoTick(g_dxySymbol,dt)||dt.time<=0){reason="WAIT_DXY_TICK";return false;}long age=(long)TimeTradeServer()-(long)dt.time;if(age<0)age=0;if(age>DxyMaxAgeSeconds){reason="WAIT_DXY_STALE";return false;}
   MqlRates d[];ArraySetAsSeries(d,true);if(CopyRates(g_dxySymbol,PERIOD_M1,1,2,d)!=2){reason="WAIT_DXY_CANDLE";return false;}double momentum=d[0].close-d[1].close;
   corr=PearsonInverseCorrelation((int)MathMax(10,DxyCorrelationBars));if(corr>1.0){reason="WAIT_DXY_CORRELATION";return false;}if(corr>MinInverseCorrelation){reason="DXY_CORRELATION_NOT_INVERSE";return false;}
   if(side=="BUY"&&momentum>=0){reason="DXY_DIRECTION_CONFLICT";return false;}if(side=="SELL"&&momentum<=0){reason="DXY_DIRECTION_CONFLICT";return false;}reason="DXY_CONFIRMED";return true;
}
...
void HandlePlaceLimit(string obj)
{
   ...
   string dxyReason="";double dxyCorr=2.0;
   if(!LocalDxyGate(side,dxyReason,dxyCorr)){Print("[Hybrid V3] WAIT_DXY setup=",setupId," side=",side," reason=",dxyReason," corr=",DoubleToString(dxyCorr,3));AckEx(id,"WAIT_DXY",0,dxyReason+" corr="+DoubleToString(dxyCorr,3),setupId,tf,side,entry,0);return;}
   Print("[Hybrid V3] DXY READY setup=",setupId," side=",side," corr=",DoubleToString(dxyCorr,3));
...
int OnInit()
{
   ...
   Print("[Hybrid V3] START ",g_symbol," AutoExecution=",EnableAutoExecution," mode=LOCAL_DXY_HARD_GATE+RAILWAY_AI FIXED_RR=1:2 NO_SL_PLUS DXY=",(g_dxySymbol==""?"OFF":g_dxySymbol));
   SendBackfill();
