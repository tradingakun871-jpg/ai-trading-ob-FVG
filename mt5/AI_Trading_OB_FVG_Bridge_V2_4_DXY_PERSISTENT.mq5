...
#property description "AI Trading OB+FVG M1/M3 Bridge V2.4 - persistent execution + DXY persistent feed"
...
bool HttpRequest(string method,string endpoint,string body,string &response)
{
   ...
   if(code==-1){Print("[Bridge V2.4] WebRequest error=",GetLastError()," URL=",url);return false;}
   response=CharArrayToString(result,0,-1,CP_UTF8);
   if(code<200||code>=300){Print("[Bridge V2.4] HTTP ",code," ",endpoint," ",response);return false;}
   return true;
}
...
string DetectDxySymbol()
{
   ...
   if(StringLen(configured)>0&&uc!="AUTO")
   {
      ...
      Print("[DXY V2.4] Configured symbol not available: ",configured,". Falling back to AUTO.");
   }
...
bool SendBackfill()
{
   ...
   if(copied<=0){Print("[Bridge V2.4] XAU backfill failed error=",GetLastError());return false;}
   ...
   if(ok)Print("[Bridge V2.4] XAU backfill OK ",copied);
   return ok;
}
...
bool SendDxyBackfill()
{
   ...
   if(copied<=0){Print("[DXY V2.4] Backfill failed symbol=",g_dxySymbol," error=",GetLastError());return false;}
   ...
   if(ok)Print("[DXY V2.4] Backfill OK symbol=",g_dxySymbol," bars=",copied);
   return ok;
}
...
bool SendClosedDxyM1()
{
   ...
   if(ok)Print("[DXY V2.4] Closed M1 sent ",g_dxySymbol," ",TimeToString(r[0].time,TIME_DATE|TIME_MINUTES));
   return ok;
}
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
{
   ...
   if(positionTicket>0&&tp1>0&&trigger>0){
      string keys[10]={"ENTRY","TP1","TP2","TP3","TP4","P1","P2","P3","P4","TRG"};
      for(int k=0;k<10;k++)if(GlobalVariableCheck(GKey(orderTicket,keys[k])))GlobalVariableSet(GKey(positionTicket,keys[k]),GlobalVariableGet(GKey(orderTicket,keys[k])));
   }
...
int OnInit()
{
   ...
   if(StringFind(BridgeToken,"PASTE_")>=0||StringLen(BridgeToken)<16){Print("[Bridge V2.4] Set BridgeToken first.");return INIT_PARAMETERS_INCORRECT;}
   ...
   if(EnableDxyFeed)
   {
      ...
      if(g_dxySymbol=="")Print("[DXY V2.4] No DXY/USDX symbol detected. Set DxySymbol manually to broker symbol.");
      else
      {
         g_currentDxyM1Open=iTime(g_dxySymbol,PERIOD_M1,0);
         Print("[DXY V2.4] ACTIVE symbol=",g_dxySymbol," backfillBars=",DxyBackfillBars," refresh=",DxyRefreshMinutes,"m");
      }
   }

   EventSetTimer((int)MathMax(1,TimerSeconds));
   Print("[Bridge V2.4 LIMIT] START ",g_symbol," AutoExecution=",EnableAutoExecution," mode=FIRST_TOUCH_LIMIT DXY=",(g_dxySymbol==""?"OFF":g_dxySymbol));
   SendBackfill();
