...
#property description "AI Trading OB+FVG M1/M3 Bridge V2.3 - persistent execution recovery + duplicate protection"
...
bool HttpRequest(string method,string endpoint,string body,string &response)
{
   ...
   if(code==-1){Print("[Bridge V2.3] WebRequest error=",GetLastError()," URL=",url);return false;}
   response=CharArrayToString(result,0,-1,CP_UTF8);
   if(code<200||code>=300){Print("[Bridge V2.3] HTTP ",code," ",endpoint," ",response);return false;}
   return true;
}
...
bool SendBackfill()
{
   ...
   if(copied<=0){Print("[Bridge V2.3] Backfill failed error=",GetLastError());return false;}
   ...
   if(ok)Print("[Bridge V2.3] Backfill OK ",copied);
   return ok;
}
...
int OnInit()
{
   ...
   if(StringFind(BridgeToken,"PASTE_")>=0||StringLen(BridgeToken)<16){Print("[Bridge V2.3] Set BridgeToken first.");return INIT_PARAMETERS_INCORRECT;}
   ...
   Print("[Bridge V2.3 LIMIT] START ",g_symbol," AutoExecution=",EnableAutoExecution," mode=FIRST_TOUCH_LIMIT");
