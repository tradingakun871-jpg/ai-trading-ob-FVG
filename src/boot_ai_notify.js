...
function aiMarketCandidate(sourceLabel,x){if(!x)return null;const bias=String(x.bias||'NEUTRAL').toUpperCase(),entry=Number(x.entryPrice||0),sl=Number(x.stopLoss||0),tp1=Number(x.takeProfit1||0),valid=bias!=='NEUTRAL'&&Number.isFinite(entry)&&entry>0&&Number.isFinite(sl)&&sl>0&&Number.isFinite(tp1)&&tp1>0;if(!valid)return null;const confidence=Number(x.confidence||0),wr=Number(x.estimatedWinrate||0),p1=Number(x.tp1Probability||0),p2=Number(x.tp2Probability||0),p3=Number(x.tp3Probability||0),score=Math.round((confidence*.40+wr*.25+p1*.20+p2*.10+p3*.05)*10)/10;return{source:sourceLabel,bias,entry,sl,score,x}}
