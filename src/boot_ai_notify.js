...
if(!source.includes('const AI_MARKET_ENTRY_UPDATE_PIPS=')){
...
const aiMarketTelegramState={intraday:{bias:null,hasEntry:false,entryPrice:null},swing:{bias:null,hasEntry:false,entryPrice:null}};
...
async function notifyAiMarketChanges(result){for(const [k,label] of [['intraday','INTRADAY'],['swing','SWING']]){const x=result?.[k];if(!x)continue;const bias=String(x.bias||'NEUTRAL').toUpperCase(),entry=Number(x.entryPrice||0),sl=Number(x.stopLoss||0),valid=bias!=='NEUTRAL'&&Number.isFinite(entry)&&entry>0&&Number.isFinite(sl)&&sl>0,prev=aiMarketTelegramState[k]||{bias:null,hasEntry:false,entryPrice:null},directionChanged=prev.bias!==null&&bias!==prev.bias,pip=Math.max(0.00001,Number(engines.M1?.config?.pipSize||0.1)),entryDeltaPips=(valid&&prev.hasEntry&&Number(prev.entryPrice)>0)?Math.abs(entry-Number(prev.entryPrice))/pip:Infinity,newEntry=valid&&(!prev.hasEntry||entryDeltaPips>=AI_MARKET_ENTRY_UPDATE_PIPS);if(valid&&(directionChanged||newEntry)){const reason=directionChanged?'PERUBAHAN_ARAH_MARKET':'ENTRY_BARU';await sendTelegram(aiMarketTelegramMessage(label,x,reason),{event:\`AI_\${label}_PLAN\`,tf:k==='intraday'?'M1/M3':'SWING'})}else if(directionChanged){await sendTelegram(aiMarketDirectionMessage(label,prev.bias,bias,x),{event:\`AI_\${label}_DIRECTION\`,tf:k==='intraday'?'M1/M3':'SWING'})}aiMarketTelegramState[k]={bias,hasEntry:valid,entryPrice:valid?entry:null}}}
`;
  source=source.slice(0,a)+replacement+source.slice(b);
  fs.writeFileSync(serverPath,source,'utf8');
}

console.log('AI Market Telegram filter active: only new entry or direction change; same-direction entry threshold='+String(process.env.AI_MARKET_ENTRY_UPDATE_PIPS||10)+' pips');
